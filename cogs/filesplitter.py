...
class FileSplitterCog(commands.Cog):
    ...
    def save_progress(self, transfer_type, channel_id, progress_data):
        try:
            if os.path.exists(PROGRESS_FILE):
                ...
                all_progress = {}
            all_progress[f"{transfer_type}_{channel_id}"] = progress_data
    ...
    async def upload_file(self, interaction: discord.Interaction, file_path: str, channel_name: str = None):
        ...
        chunks_uploaded = len(completed_chunks)
        last_percentage = 0
                    ...
                    for i in range(total_parts):
                        ...
                        while retries > 0:
                            try:
                                ...
                                if not chunk:
                                    break
                                chunk_file = discord.File(fp=io.BytesIO(chunk), filename=chunk_id)
                                await new_channel.send(f"📦 `{rel_path}` - Part {i+1}/{total_parts}", file=chunk_file)
                                await asyncio.sleep(1)
                                chunk_uploaded_flag = True
                                uploaded_bytes += len(chunk)  # Track actual bytes uploaded
                                break
                            except discord.errors.HTTPException as e:
                                retries -= 1
                                await new_channel.send(f"⌐ Error with part {i+1}: {e}. Retrying... ({retries} left)")
    ...
    async def download_file(self, interaction: discord.Interaction, channel_name: str = None):
                ...
                for attachment in message.attachments:
                    ...
                    if '.part_' in filename and '_of_' in filename:
                        try:
                            ...
                            part_info = filename.split('.part_')[1]
                            
                            # Parse part_X_of_Y
                            part_match = re.match(r'(\d+)_of_(\d+)', part_info)
                            if not part_match:
                                continue
        ...
        for file_path, file_chunks in chunks.items():
            ...
            try:
                    ...
                    for part_num in range(resume_part, total_parts + 1):
                        ...
                        while retries > 0:
                            try:
                                ...
                                retries -= 1
                                if retries > 0:
                                    await target_channel.send(f"⚠️ Error downloading part {part_num}, retrying... ({retries} left)")
                                    await asyncio.sleep(5)
                                else:
                ...
                await target_channel.send(f"✅ Completed `{file_path}`")
